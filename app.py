    ...
    try:
        ...
        if results and len(results) > 0:
            ...
            if result.boxes is not None:
                ...
                # create grid center coordinates tensor
                ...
                # Use to store added grid positions, avoid duplicates
                added_positions = set()
                ...
                for i in range(len(boxes)):
                    ...
                    # Use CUDA acceleration
                    # move the box coordinates to GPU
                    ...
                    # check if all the grids in the box
                    ...
                    # get the indices of the grid points inside the box
                    inside_indices = torch.nonzero(in_box).flatten()

                    # record if a grid point is found
                    found_grid_points = len(inside_indices) > 0

                    # transform to grid coordinates
                    for idx in inside_indices:
                        a = idx // N  # column coordinate (0 to M-1)
                        b = idx % N   # row coordinate (0 to N-1)

                        if 0 <= a < M and 0 <= b < N:
                            # check duplication
                            position_key = (a.item(), b.item())  # change to python value
                            if position_key not in added_positions:
                                grid_positions.append({
                                    'id': i,
                                    'a': a.item(),  # change to python value
                                    'b': b.item()
                                })
                                added_positions.add(position_key)
                    
                    # If no grid points are found, use the center point as the grid position
                    ...
                    # calculate the original center grid
                    center_grid_a = int(center_x // grid_width)  # column coordinate (0 to M-1)
                    center_grid_b = int(center_y // grid_height)  # row coordinate (0 to N-1)
